...
class Program
{
    static void Main(string[] args)
    {
        UpdateConfig("./config.txt",
                     new Dictionary<string, string> { { "something.connection", DateTime.Now.Ticks.ToString() } });
    }

    static void UpdateConfig(string path, Dictionary<string, string> values)
    {
        var content = values.Aggregate(File.ReadAllText(path),
                                       (acc, kvp) => Regex.Replace(acc, $"^{kvp.Key}=.*", $"{kvp.Key}={kvp.Value}"));
...
//         public static List<T> Print<T>(this List<T> t)
//         {
